.PHONY: help run build tidy fmt vet lint install-deps test docker-up docker-down docker-restart docker-logs docker-ps

BINARY := build/app/raccodown
...
run: ## Run the application (go run ./cmd/raccodown)
	go run ./cmd/raccodown

build: ## Build binary for Linux (amd64) into build/app/raccodown
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o $(BINARY) ./cmd/raccodown
