...
func (s *Storage) CreateNote(ctx context.Context, note model.Note) (uint64, error) {
	...
	res, err := tx.ExecContext(ctx,
		`INSERT INTO notes (title, content, checksum, created_at, updated_at) VALUES (?, ?, ?, ?, ?)`,
...
func (s *Storage) UpdateNote(ctx context.Context, note model.Note) (bool, error) {
	...
	res, err := tx.ExecContext(ctx,
		`UPDATE notes SET title = ?, content = ?, checksum = ?, updated_at = ? WHERE id = ?`,
...
// left alone — INSERT IGNORE is MySQL's equivalent of SQLite's INSERT ... ON CONFLICT DO NOTHING)
// and links noteID to all of them. Runs against a transaction the caller controls (CreateNote/
// UpdateNote), so the note row and its tags land atomically.
func setNoteTags(ctx context.Context, tx *sql.Tx, noteID uint64, tags []string) error {
	now := time.Now().UTC()

	for _, tag := range tags {
		if _, err := tx.ExecContext(ctx,
			`INSERT IGNORE INTO tags (name, created_at) VALUES (?, ?)`, tag, now,
		); err != nil {
			return fmt.Errorf("upserting tag %q: %w", tag, err)
		}

		if _, err := tx.ExecContext(ctx,
			`INSERT INTO note_tags (note_id, tag_name) VALUES (?, ?)`, noteID, tag,
		); err != nil {
...
func (s *Storage) tagsForNote(ctx context.Context, noteID uint64) ([]string, error) {
	rows, err := s.DB.QueryContext(ctx,
		`SELECT tag_name FROM note_tags WHERE note_id = ? ORDER BY tag_name`, noteID,
	)
	if err != nil {
		return nil, err
	}
