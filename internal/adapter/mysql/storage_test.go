...
// mysqlErr builds a *mysqldriver.MySQLError carrying number — the field wrapUnique switches on —
...
// migrations/00001_init.sql) don't carry fractional seconds, so keeping sub-second precision out of
// the fixture avoids tests asserting a precision the schema doesn't have.
...
// TestInsertReturningID checks that the generated id comes back from LastInsertId, and that a
// failing statement reports the error instead of a zero id.
func TestInsertReturningID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(query).WithArgs(name, createdAt).WillReturnResult(sqlmock.NewResult(wantID, 1))
			},
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(query).WithArgs(name, createdAt).WillReturnError(errStub)
			},
			assertResult: func(t *testing.T, got uint64) {},
			assertErr:    func(t *testing.T, err error) { require.ErrorIs(t, err, errStub) },
		},
...
// still wrapped inside. Every other error (including nil) has to pass through untouched, otherwise a
// repository would report "already taken" for an unrelated failure.
func TestWrapUnique(t *testing.T) {
	t.Parallel()

	duplicate := mysqlErr(errDuplicateEntry)
	...
	tests := []struct {
		...
		{
			name: "a different MySQL error number passes through",
			err:  mysqlErr(1451),
			assertResult: func(t *testing.T, in, got error) {
				require.ErrorIs(t, got, in)
				require.NotErrorIs(t, got, storageError.UniqueViolationError)
