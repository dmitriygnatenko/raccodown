...
func TestHandleUpdateSettings(t *testing.T) {
	...
	t.Run("blank language fails validation with 400", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodPatch, "/api/v1/settings", strings.NewReader(`{"language":"","theme":"light"}`))
		req.AddCookie(authCookie())
		rec := httptest.NewRecorder()
	...
	t.Run("blank theme fails validation with 400", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodPatch, "/api/v1/settings", strings.NewReader(`{"language":"fr","theme":""}`))
		req.AddCookie(authCookie())
		rec := httptest.NewRecorder()
	...
	t.Run("an invalid theme fails validation with 400", func(t *testing.T) {
		t.Parallel()

		mux, deps := newTestServer()
		stubAuthenticated(deps, entity.User{ID: 1})

		req := httptest.NewRequest(http.MethodPatch, "/api/v1/settings", strings.NewReader(`{"language":"fr","theme":"sepia"}`))
		req.AddCookie(authCookie())
		rec := httptest.NewRecorder()
	...
	t.Run("valid request updates and returns 200", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodPatch, "/api/v1/settings", strings.NewReader(`{"language":"fr","theme":"dark"}`))
		req.AddCookie(authCookie())
		rec := httptest.NewRecorder()
