...
func TestFindNoteByID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(noteSelect + ` WHERE id = $1`).WithArgs(note.ID).WillReturnRows(
				...
				mock.ExpectQuery(`SELECT tag_name FROM note_tags WHERE note_id = $1 ORDER BY tag_name`).
					WithArgs(note.ID).
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(noteSelect + ` WHERE id = $1`).WithArgs(note.ID).WillReturnError(sql.ErrNoRows)
			},
			assertResult: func(t *testing.T, got model.Note) {},
...
func TestListNotes(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT note_id, tag_name FROM note_tags WHERE note_id IN ($1,$2) ORDER BY note_id, tag_name`).
		WithArgs(a.ID, b.ID).
...
func TestCreateNote(t *testing.T) {
	...
	insertNote := `INSERT INTO notes (title, content, checksum, created_at, updated_at) VALUES ($1, $2, $3, $4, $5) RETURNING id`
	insertTag := `INSERT INTO tags (name, created_at) VALUES ($1, $2) ON CONFLICT (name) DO NOTHING`
	linkTag := `INSERT INTO note_tags (note_id, tag_name) VALUES ($1, $2)`
	...
	t.Run("inserts the note and its tags, returning the new id", func(t *testing.T) {
		...
		mock.ExpectExec(linkTag).WithArgs(wantID, note.Tags[0]).WillReturnResult(sqlmock.NewResult(0, 1))
		mock.ExpectCommit()
...
func TestUpdateNote(t *testing.T) {
	...
	updateNote := `UPDATE notes SET title = $1, content = $2, checksum = $3, updated_at = $4 WHERE id = $5`
	deleteLinks := `DELETE FROM note_tags WHERE note_id = $1`
	insertTag := `INSERT INTO tags (name, created_at) VALUES ($1, $2) ON CONFLICT (name) DO NOTHING`
	linkTag := `INSERT INTO note_tags (note_id, tag_name) VALUES ($1, $2)`
