...
// for generated ids). The queries live next to this file, one file per table group (note.go, user.go,
// session.go).
package postgres
...
// pgUniqueViolation is the Postgres SQLSTATE code raised when a UNIQUE constraint is violated.
