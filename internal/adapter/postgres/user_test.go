...
// which has to arrive as storageError.UniqueViolationError and not as a raw driver error. Unlike
// MySQL/SQLite, the new id comes back via a RETURNING clause, scanned like a query row rather than
// read off a Result.
func TestCreateUser(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(query).WithArgs(username, hash, createdAt, updatedAt).
		...
		},
		{
			name: "a driver error is propagated",
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(query).WithArgs(username, hash, createdAt, updatedAt).WillReturnError(errStub)
			},
			assertResult: func(t *testing.T, got uint64) {},
			assertErr:    func(t *testing.T, err error) { require.ErrorIs(t, err, errStub) },
		},
...
func TestUpdateUserCredentials(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(`UPDATE users SET username = $1, updated_at = $2 WHERE id = $3`).
		...
		},
		{
			...
			passwordHash: hash,
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(`UPDATE users SET username = $1, password_hash = $2, updated_at = $3 WHERE id = $4`).
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(`UPDATE users SET username = $1, updated_at = $2 WHERE id = $3`).
					WithArgs(newUsername, updatedAt, id).WillReturnError(pgErr(pgUniqueViolation))
			},
			assertResult: func(t *testing.T, got bool) {},
			assertErr: func(t *testing.T, err error) {
				require.ErrorIs(t, err, storageError.UniqueViolationError)
...
func TestUpdateUserSettings(t *testing.T) {
	...
	query := `UPDATE users SET settings = $1, updated_at = $2 WHERE id = $3`
	userID := fakeID()
	settings := fakeUserSettings()
	updatedAt := fakeTime()
