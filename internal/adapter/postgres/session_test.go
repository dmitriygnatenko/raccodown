...
// already in use) surfaces as a plain *pgconn.PgError, not one of the storage package's sentinels.
func TestCreateSession(t *testing.T) {
	t.Parallel()

	query := `INSERT INTO sessions (token, user_id, expires_at, created_at) VALUES ($1, $2, $3, $4)`
...
func TestFindSessionByToken(t *testing.T) {
	...
	query := `SELECT user_id, expires_at, created_at FROM sessions WHERE token = $1`
	token := fakeToken()
	userID := fakeID()
	expiresAt, createdAt := fakeTime(), fakeTime()
