// Package app wires up the application's dependencies (config, repositories, use cases, HTTP server)
// and runs it. It's the composition root, kept separate from cmd/raccodown/main.go so main can stay
// a thin entry point.
package app
...
// Run loads configuration, wires up the repository, use cases and HTTP server, and blocks serving
// requests until the process is asked to stop.
func Run() error {
	time.Local = time.UTC
	...
	if !appCfg.CookieSecure {
		slog.Warn("COOKIE_SECURE is false: the session cookie is sent without the Secure flag, " +
			"so it can be intercepted over an unencrypted connection — set COOKIE_SECURE=true " +
	...
	if err = seedDemoNotes(ctx, repos.Notes); err != nil {
		return fmt.Errorf("failed to seed demo notes: %w", err)
	}

	if err = seedDemoUser(ctx, seedDemoUserParams{
		Users:    repos.Users,
		Hasher:   svcs.Hasher,
