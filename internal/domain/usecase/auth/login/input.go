...
// Input is what LoginUser needs to verify credentials and start a session. Language is the
// frontend's current UI language — saved as the user's preference the first time they ever log in
// with one on record, and otherwise ignored (see UseCase.Execute).
type Input struct {
	Username string
	Password string
	Language string
