// Package authenticate is the AuthenticateSession use case: it resolves a session token into the
// user it belongs to, used by the adapter/http requireAuth middleware.
...
func New(sessionRepository port.SessionRepository, userRepository port.UserRepository) *UseCase {
	return &UseCase{
...
func (uc *UseCase) Execute(ctx context.Context, input Input) (Output, error) {
	...
	if err != nil {
		if !domainError.IsNotFoundError(err) {
			slog.ErrorContext(ctx, "authenticate: find session", "error", err)
		}
	...
	if session.ExpiresAt.Before(time.Now().UTC()) {
		slog.InfoContext(ctx, "authenticate: expired session")

		if err = uc.sessionRepository.Delete(ctx, input.Token); err != nil {
			slog.ErrorContext(ctx, "authenticate: delete expired session", "error", err)
		}
	...
	return Output{User: user.Public()}, nil
}
