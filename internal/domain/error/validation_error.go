...
// structurally fine but fails a referential check, while Fields carries the field name -> error map
// that validation.ValidateStruct produces for form-shaped input. Error() renders either one, so
// callers that only need text can ignore the difference.
...
// message (Go map iteration order is random). A lone field error is returned verbatim, and only the
// multi-error case gets joined into "One. Two.".
