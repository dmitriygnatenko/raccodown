...
export default {
  ...
  template: `
    <section class="workspace" v-if="note">
      <header class="toolbar">
        <button class="back-btn" :title="t('К списку заметок')" @click="back">
  ...
  methods: {
    ...
    // Only reachable on narrow screens (see .back-btn in style.css) — returns to the note list
    // without changing which note is selected on wider screens where both panes show at once.
    back() {
      notesStore.select(null)
    },
  },
}
